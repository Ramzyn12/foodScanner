...
const SearchBar = () => {
  ...
  const { data: DataOFF, refetch: refetchOFF } = useQuery({
    ...
    queryFn: () => fetchFoodWithSearch(searchInput),
  ...
  useEffect(() => {
    if (DataOFF) {
      dispatch(setOpenFoodFactsResults(DataOFF));
    } else {
      dispatch(setOpenFoodFactsResults([]));
    }
  }, [DataOFF, dispatch]);
  ...
  const handleSearchChange = (input) => {
    ...
    debouncedRefetch(input); // Use the debounced function to control the API call
  };

  // Handle pressing the (x) button
  const handleClearInput = () => {
    setSearchInput("");
    // dispatch(setSearchResults([])); // Clear search results in the store
    setShowClear(false);
  };
